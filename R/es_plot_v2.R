...
es_plot_v2 <- function(effect_size, ci_lower, ci_upper, palette = c("viridis", "magma", "plasma", "cividis", "grey"),
                                      arrow_color = "black", reverse = FALSE,
                                      save_path = NULL, axis_title_size = 12, axis_label_size = 10) {
  ...
  # Create a grid of points only between CI angles
  ...
  grid_data$z <- get_palette(palette, length(unique(grid_data$th)))[
    as.numeric(cut(distance_from_estimate, breaks = 250))
  ]
  ...
  p <- ggplot2::ggplot(grid_data) +
    ggplot2::geom_tile(ggplot2::aes(x = x, y = y, fill = z), width = 0.01, height = 0.01) +  # Faster tile-based shading
    ...
    ggplot2::coord_fixed() +
    ggplot2::theme_void() +
    ggplot2::annotate("text", x = -.2, y = 0, label = "Effect size (95% CI)", angle = 90, hjust = 0.5, size = axis_title_size) +  # Axis title
